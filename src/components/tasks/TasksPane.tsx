...
type PendingSaveType = "normal" | "recurring-this" | "recurring-all" | "recurring-from-today" | null;
...
export const TasksPane: React.FC = () => {
  ...
  const performStateChange = (taskId: string, task: Task, newState: TaskState, thisOnly: boolean) => {
              ...
              if (selectedTask.parentId) {
                ...
                for (const [date, taskList] of Object.entries(tasks)) {
                  ...
                  if (rootIndex !== -1) {
                    ...
                    const findSubtaskByTitlePath = (task: Task, targetTask: Task, path: string[] = []): string[] | null => {
                    ...
                    const deleteByTitlePath = (task: Task, titlePath: string[], currentDepth: number = 0): Task => {
            ...
            onConfirm: (choice: "this" | "all" | "from-today") => {
              ...
              if (choice === "this") {
                ...
                if (taskBeingEdited.parentId) {
                  ...
                  const updateSubtaskInTree = (task: Task, targetId: string, newTitle: string): Task => {
                    if (task.id === targetId) {
